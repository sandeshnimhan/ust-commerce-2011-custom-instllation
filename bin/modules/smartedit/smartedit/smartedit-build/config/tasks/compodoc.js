...
module.exports = function(grunt) {
    ...
    // Compodoc has issues with custom decorators usage with Angular build-it ones. We need to
    // use a walkaround to remove these decorators before compodoc evaluation.
    ...
    function removeLegacyFiles() {
        getFilesRecursively(configPaths.compodoc.temp)
            .filter(includeTsFile)
            .filter(includeByRegexp(LEGACY_DECORATOR_REGEXP))
            .forEach((path) => removeDirectoryOrFile(path));
    }
    ...
    function generateCompodoc(input, output, title, async) {
        ...
        removeLegacyFiles();
        startCompodoc(input, output, title, async);
    }

    function startCompodoc(input, output, name, done) {
